...
def is_threed_layer(norm_layer):
    if norm_layer.func.__name__.find("3d") > -1:
        return True
    else:
        return False


def add_bias(norm_layer, threed):
    # no need to use bias as BatchNorm2d has affine parameters
    if type(norm_layer) == functools.partial:
        comp = norm_layer.func
    else:
        comp = norm_layer

    if threed:
        return comp == nn.InstanceNorm3d
    else:
        return comp == nn.InstanceNorm2d


def get_norm_layer(norm_type='instance', threed=False):
    ...
    """
    if norm_type == 'batch':
        batchnorm = nn.BatchNorm3d if threed else nn.BatchNorm2d
        norm_layer = functools.partial(batchnorm, affine=True, track_running_stats=True)
    elif norm_type == 'instance':
        instancenorm = nn.InstanceNorm3d if threed else nn.InstanceNorm2d
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    if netG == 'resnet_9blocks':
        ...
        net = UnetGenerator(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, upsampling=upsampling,
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, upsampling='deconvolution',
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [conv_layer(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [conv_layer(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [conv_layer(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim)]
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, upsampling='deconvolution',
        ...
        unet_block = uskipblock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer,
                                innermost=True, layer_index=layer_index)  # add the innermost layer
        layer_index += 1
        for i in range(num_downs - 5):  # add intermediate layers with ngf * 8 filters
            unet_block = uskipblock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block,
                                    norm_layer=norm_layer, use_dropout=use_dropout, layer_index=layer_index)
            layer_index += 1
            
        # gradually reduce the number of filters from ngf * 8 to ngf
        unet_block = uskipblock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block,
                                norm_layer=norm_layer, layer_index=layer_index)
        layer_index += 1
        unet_block = uskipblock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block,
                                norm_layer=norm_layer, layer_index=layer_index)
        layer_index += 1
        unet_block = uskipblock(ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer, layer_index=layer_index)
        layer_index += 1
        self.model = uskipblock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True,
                                norm_layer=norm_layer, layer_index=layer_index)  # add the outermost layer
        layer_index += 1
...
class LinearUpsampleUnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None, submodule=None, outermost=False, innermost=False,
                 norm_layer=nn.BatchNorm2d, use_dropout=False, n_splits=4, layer_index=-1):
        ...
        if input_nc is None:
            input_nc = outer_nc
        # downconv = conv_layer(input_nc, inner_nc, kernel_size=4, stride=2, padding=1, bias=use_bias)
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            ...
            upconv = conv_trans_layer(inner_nc, outer_nc,
                                      kernel_size=4, stride=2,
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        sequence += [
            conv_layer(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
