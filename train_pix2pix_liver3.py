...
save_path = './pix2pix_model/'+time.strftime("%Y%m%d-%H%M%S"+'-pix2pix-liver2')
if not os.path.exists(save_path):
    os.mkdir(save_path) 

##### Initialize logging #####
logger = wandb.init(project='pix2pix_train', name="liver2", resume='allow', anonymous='must')
    ...
    for i, data in enumerate(train_loader):  # inner loop within one epoch
        ...
        if total_iters % opt.display_freq == 0:
            ...
            for label, image in visuals.items():
                if label == 'pix2pix_real_mask':
                    image_numpy = image[0].mul(122).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
                else:
