...
save_path = './checkpoint_breast/'+'end2end-fetoscopy-100-'+str(opt.seg_model)+'-'+str(opt.loss_lambda)+'-'+time.strftime("%Y%m%d-%H%M%S")
...
logger = wandb.init(project='end2end-fetoscopy', name=str(opt.seg_model)+"-100-"+str(opt.loss_lambda), resume='allow', anonymous='must')
...
model_path = './pix2pix_model/20230414-062134-pix2pix-fetoscopy-100'
...
##### prepare dataloader #####
dataset = BasicDataset(opt.dataroot+'/Images', opt.dataroot+'/Masks', 1.0, '_mask')
...
class Unet(ImplicitProblem):
    def training_step(self, batch):
        ...
        fake_mask = (true_masks*255).to('cpu', torch.uint8).numpy()
        if opt.batch_size > 1:
            for i in range(opt.batch_size):
                fake_mask[i] = seq(images=fake_mask[i])
        fake_mask = torch.tensor(fake_mask/255.0).type(torch.cuda.FloatTensor).to(device=device)
