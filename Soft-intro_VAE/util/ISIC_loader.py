...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = self.load(mask_file[0]).convert('L').resize((128, 128), resample=Image.NEAREST)
        img = self.load(img_file[0]).convert('RGB').resize((128, 128))
        
        img = self.preprocess(img, self.scale, is_mask=False)
        mask = self.preprocess(mask, self.scale, is_mask=True)

        mask_pix2pix = self.load_pix2pix(mask_file[0]).convert('L').resize((128, 128))
        img_pix2pix = self.load_pix2pix(img_file[0]).convert('RGB').resize((128, 128))
