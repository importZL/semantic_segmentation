...
save_path = './checkpoint_liver/'+'end2end-liver2-'+str(opt.seg_model)+'-'+str(opt.loss_lambda)+'-'+time.strftime("%Y%m%d-%H%M%S")
...
logger = wandb.init(project='end2end-liver', name=str(opt.seg_model)+"-liver2-"+str(opt.loss_lambda), resume='allow', anonymous='must')
...
model_path = '/home/li/workspace/semantic_segmentation/pix2pix_model/20230428-065551-pix2pix-liver2'
...
class Unet(ImplicitProblem):
    def training_step(self, batch):
        ...
        loss += dice_loss(
            ...
            F.one_hot(true_masks.squeeze(1), opt.classes).permute(0, 3, 1, 2).float(),
            multiclass=True
        )
        '''
        masks_pred = net(images)
        ...
        fake_mask = fake_mask.squeeze(0).long()
        ...
        fake_loss = criterion(fake_pred, fake_mask.squeeze(1))
        fake_loss += dice_loss(
            F.softmax(fake_pred, dim=1).float(),
            F.one_hot(fake_mask.squeeze(1), opt.classes).permute(0, 3, 1, 2).float(),
            multiclass=True
        )
        '''
        fake_pred = net(fake_image)
        fake_loss = criterion(fake_pred.squeeze(0), fake_mask.float())
...
class Arch(ImplicitProblem):
    def training_step(self, batch):
        ...
        image_valid = batch['image'].to(device=device, dtype=torch.float32)
        mask_pred = self.unet(image_valid)
        '''
        loss_arch = criterion(mask_pred, mask_valid.squeeze(1))
        loss_arch += dice_loss(
            F.softmax(mask_pred, dim=1).float(),
            F.one_hot(mask_valid.squeeze(1), opt.classes).permute(0, 3, 1, 2).float(),
            multiclass=True
        )
        '''
