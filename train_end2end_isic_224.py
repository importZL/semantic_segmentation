...
from util.ISIC_224_loader import CarvanaDataset as BasicDataset
...
save_path = './checkpoint_skin_new/'+'end2end-ISIC-40-'+str(opt.seg_model)+'-'+str(opt.loss_lambda)+'-'+time.strftime("%Y%m%d-%H%M%S")
...
model_path = './pix2pix_model/20230314-210809-pix2pix-UNet-40' #'./pix2pix_model/20230320-194227-pix2pix-UNet-100' # './pix2pix_model/20230315-055526-pix2pix-UNet-200'
...
class Unet(ImplicitProblem):
    def training_step(self, batch):
        ...
        generate_mask = batch['mask_pix2pix'].to(device=device, dtype=torch.long).squeeze(0)
        fake_mask = generate_mask.to('cpu', torch.float).numpy()
        if opt.batch_size > 1:
        ...
        # resize to 224 * 224
        fake_mask = transforms.functional.resize(fake_mask, 224)
        fake_image = transforms.functional.resize(fake_image, 224)
