...
conv_arch = Variable(1e-3*torch.randn(21, num_ops_conv), requires_grad=True)
    ...
    def __init__(
        ...
        if is_unet:
            self.in_layers = nn.Sequential(
                ...
                nn.SiLU(),
                Cell_conv(channels, self.out_channels, cell_index),
            )
            cell_index += 1
        else:
            self.in_layers = nn.Sequential(
        ...
        if False: #is_unet:
            self.out_layers = nn.Sequential(
                ...
                zero_module(
                    Cell_conv(self.out_channels, self.out_channels, cell_index)
                ),
            )
            cell_index += 1
        else:
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        #     [
        #         TimestepEmbedSequential(
        #             Cell_conv(in_channels, model_channels, cell_index)
        #         )
        #     ]
        # )
        # cell_index += 1
        ...
        #     normalization(ch),
        #     nn.SiLU(),
        #     zero_module(Cell_conv(model_channels, out_channels, cell_index)),
        # )
        # cell_index += 1
