...
class BaseRunner(ABC):
    ...
    def train(self):
        ...
        train_dataset_all, test_dataset = get_dataset1(self.config.data)
        indices = list(range(len(train_dataset_all)))
        num_train = int(len(train_dataset_all)/2)
        train_dataset = Subset(train_dataset_all, indices[:num_train])
        val_dataset = Subset(train_dataset_all, indices[num_train:])
