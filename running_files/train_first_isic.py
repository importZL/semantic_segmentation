...
save_path = './checkpoint/'+'end2end_ISIC-first-200-'+str(opt.loss_lambda)+'-'+time.strftime("%Y%m%d-%H%M%S")
...
logger = wandb.init(project='end2end-ISIC', name="unet-first-200-"+str(opt.loss_lambda), resume='allow', anonymous='must')
...
while total_iters < opt.n_epochs:
    for i, batch in enumerate(train_loader):
        ...
        train_image = batch['image_pix2pix']
        train_mask = batch['mask_pix2pix']
        ...
        mask_valid = valid_data['mask'].type(torch.cuda.FloatTensor).to(device).squeeze(0)
        image_valid = valid_data['image'].type(torch.cuda.FloatTensor).to(device)
