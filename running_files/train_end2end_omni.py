...
save_path = './checkpoint_omni/'+'end2end-worm-30-'+str(opt.seg_model)+'-'+str(opt.loss_lambda)+'-'+time.strftime("%Y%m%d-%H%M%S")
...
logger = wandb.init(project='end2end-omni', name=str(opt.seg_model)+"-worm-30-"+str(opt.loss_lambda), resume='allow', anonymous='must')
...
model_path = '/home/li/workspace/semantic_segmentation/pix2pix_omni_model/20230331-022004-omni-30'
...
class Generator(ImplicitProblem):
    def training_step(self, batch):
        real_mask = batch['mask'].unsqueeze(1).type(torch.cuda.FloatTensor).to(device)
...
class Unet(ImplicitProblem):
    def training_step(self, batch):
        ...
        loss = criterion(masks_pred.squeeze(), true_masks.float())
        loss += dice_loss(torch.sigmoid(masks_pred.squeeze()), true_masks.float().squeeze())

        # fake images and masks
        fake_mask = true_masks.unsqueeze(1).to('cpu', torch.float).numpy()
        if opt.batch_size > 1:
        ...
        fake_loss = criterion(fake_pred.squeeze(), fake_mask.squeeze().float())
        ...
        if show_index % int(len(train_set)) == 0:
            ims_dict = {}
            show_image = images[0].mul(255).add_(0.5).clamp_(0, 255).to('cpu').numpy()
            show_mask = true_masks[0].mul(255).to('cpu').numpy()
            show_fake_image = fake_image[0].mul(255).add_(0.5).clamp_(0, 255).to('cpu').detach().numpy()
            show_fake_mask = fake_mask[0].mul(255).to('cpu').numpy()
...
class Arch(ImplicitProblem):
    def training_step(self, batch):
        ...
        loss_arch = criterion(mask_pred.squeeze(), mask_valid.float())
