...
save_path = './pix2pix_model/'+time.strftime("%Y%m%d-%H%M%S"+'-pix2pix-opti')
if not os.path.exists(save_path):
    os.mkdir(save_path) 

##### Initialize logging #####
# logger = wandb.init(project='end2end-unet-ISIC', name="unet-200", resume='allow', anonymous='must')
logger = wandb.init(project='pix2pix_train', name="opti", resume='allow', anonymous='must')
...
for epoch in range(1000):
    ...
    for i, data in enumerate(train_loader):  # inner loop within one epoch
        ...
        valid_mask = valid_data['mask'].unsqueeze(1)
