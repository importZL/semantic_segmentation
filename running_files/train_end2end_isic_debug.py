...
save_path = './checkpoint_debug/'+'end2end-ISIC-40-'+str(opt.seg_model)+'-'+str(opt.loss_lambda)+'-'+time.strftime("%Y%m%d-%H%M%S")
...
logger = wandb.init(project='end2end-ISIC-debug', name=str(opt.seg_model)+"-40-"+str(opt.loss_lambda), resume='allow', anonymous='must')
...
problems = [unet] #[netG, netD, unet, arch]
