...
save_path = './checkpoint_opti/'+'UNet_JSRT-100-'+time.strftime("%Y%m%d-%H%M%S")
...
logger = wandb.init(project='opti', name="unet", resume='allow', anonymous='must')
...
##### prepare dataloader #####
dataset = DiabeticDataset('/home/li/workspace/data/Diabetic-eye/train')
...
while total_iters < opt.n_epochs:
    for i, batch in enumerate(train_loader):
        ...
        loss = criterion(masks_pred.squeeze(0), true_masks.long())
        loss += dice_loss(torch.sigmoid(masks_pred), F.one_hot(true_masks, opt.classes).permute(0, 3, 1, 2).long(), multiclass=True)
