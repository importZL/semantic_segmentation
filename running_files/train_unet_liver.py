...
save_path = './checkpoint_liver/'+str(opt.seg_model)+'_liver2-'+time.strftime("%Y%m%d-%H%M%S")
if not os.path.exists(save_path):
    os.mkdir(save_path) 
unet_save_path = save_path+'/unet.pkl'  

##### Initialize logging #####
logger = wandb.init(project='vanilla-unet', name=str(opt.seg_model)+"-liver2", resume='allow', anonymous='must')
