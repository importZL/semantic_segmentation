...
save_path = './checkpoint_kvasir/'+'end2end-kvasir-400-'+str(opt.seg_model)+'-'+str(opt.loss_lambda)+'-'+time.strftime("%Y%m%d-%H%M%S")
if not os.path.exists(save_path):
    os.mkdir(save_path) 
unet_save_path = save_path+'/'+str(opt.seg_model)+'.pkl'  

##### Initialize logging #####
logger = wandb.init(project='end2end-kvasir', name=str(opt.seg_model)+"-400-"+str(opt.loss_lambda), resume='allow', anonymous='must')
...
model_path = './pix2pix_model/20230424-215847-pix2pix-kvasir'
