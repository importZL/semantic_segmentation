...
save_path = './pix2pix_JSRT_model/'+time.strftime("%Y%m%d-%H%M%S"+'-pix2pix-Cell-100')
if not os.path.exists(save_path):
    os.mkdir(save_path) 

##### Initialize logging #####
logger = wandb.init(project='pix2pix_train', name="Cell-100", resume='allow', anonymous='must')
...
loader_args = dict(batch_size=opt.batch_size, num_workers=0, pin_memory=True)
