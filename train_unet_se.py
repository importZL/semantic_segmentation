...
save_path = './checkpoint_se/'+'UNet_100-'+time.strftime("%Y%m%d-%H%M%S")+'-flip'
...
logger = wandb.init(project='vanilla-unet', name="SE-100", resume='allow', anonymous='must')
...
while total_iters < opt.n_epochs:
    for i, batch in enumerate(train_loader):
        ...
        fake_masks_pred = net(fake_image)
        fake_loss = criterion(fake_masks_pred.squeeze(0), fake_mask.float())
        fake_loss += jaccard_index_loss(torch.sigmoid(fake_masks_pred.squeeze()), fake_mask.float().squeeze())
