...
class ResNet(nn.Module):
    def __init__(self, block, layers, output_stride, BatchNorm, pretrained_url=None):
        self.inplanes = 64
        ...
        if self.pretrained_url is not None:
    ...
    def _init_weight(self):
        for m in self.modules():
            ...
            elif isinstance(m, pixelssl.SynchronizedBatchNorm2d):
    ...
    def _load_pretrained_model(self):
        if os.path.isfile(self.pretrained_url):
            self.load_state_dict(torch.load(self.pretrained_url))
        else:
